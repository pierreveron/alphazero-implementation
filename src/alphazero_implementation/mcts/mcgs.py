...
class Node:
    ...
    Attributes:
        ...
        hash (int): A hash value of the game state, used for efficient node lookup
            and comparison.
...
def select_action_according_to_puct(node: Node, c_puct: float = 1.0) -> Action:
    ...
    for action in node.game_state.actions:
        ...
        puct_value = node.game_state.player * Q_value + c_puct * P_value * math.sqrt(
            total_visits
        ) / (1 + N_a)
...
def perform_one_playout(node: Node):
    if is_game_over(node):
        ...
        if action not in node.children_and_edge_visits:
            ...
            new_hash = hash(new_game_state)
            if new_hash in nodes_by_hash:
                child = nodes_by_hash[new_hash]
